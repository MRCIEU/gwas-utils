...
fn run() -> Result<(), Box<dyn Error>> {
    let (ifilenames, file_wtr) = handle_commandline_args()?;
    process_files(ifilenames, file_wtr)?;
    Ok(())
}

fn handle_commandline_args() -> Result<(Vec<String>, gwas_utilities::Writer), Box<dyn Error>> {
    let args = Args::parse();
